...
BEGIN_EVENT_TABLE(BoardPanel, wxPanel)
	...
	EVT_KEY_DOWN(BoardPanel::OnKeyDown)
	EVT_TIMER(wxID_ANY, BoardPanel::OnTimer)
END_EVENT_TABLE()


BoardPanel::BoardPanel(wxFrame *parent) : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxBORDER_NONE) {
...
void BoardPanel::OnTimer(wxTimerEvent& event)
{
...
void BoardPanel::ClearBoard()
{
    for (int i = 0; i < BoardHeight * BoardWidth; ++i)
        board[i] = NoShape;
}

void BoardPanel::DropDown()
{
    int newY = curY;
    while (newY > 0) {
...
void BoardPanel::OneLineDown()
{
    if (!TryMove(curPiece, curX, curY - 1))
        PieceDropped();
}

void BoardPanel::PieceDropped()
{
    for (int i = 0; i < 4; ++i) {
        int x = curX + curPiece.x(i);
        int y = curY - curPiece.y(i);
...
void BoardPanel::RemoveFullLines()
{
     int numFullLines = 0;
...
bool BoardPanel::TryMove(const Shape& newPiece, int newX, int newY)
{
    for (int i = 0; i < 4; ++i) {
...
void BoardPanel::DrawSquare(wxPaintDC& dc, int x, int y, Tetrominoes shape)
{
